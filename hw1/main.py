...
def place_bandits(maze,bandit_cnt,prob:float,gold:int=0,history=[]):
    ...
    subsets = list(itertools. combinations(s, bandit_cnt))
    childrens = []
    actions = []
    for subset in subsets:
        ...
        childrens.append(build_player(maze,bandit_cnt,subset,prob,gold,history+[action]))
...
def build_player(maze,bandit_cnt:int,bandit_pos:Tuple,prob:float,gold:int=0,history=[]):
    ...
    #player_history = tuple([h for h in history if h != 'fight' and h != 'let_go' and str.isnumeric(h)])
    info = tuple([tuple(maze.reshape(-1).tolist()),gold]) #(Need to add results of fights with bandits)
    #info = tuple(history)
    infoset = hash(info) 
...
def build_bandit(maze,bandit_cnt:int,bandit_pos:tuple,prob:float,gold:int=0,history=[]):
    ...
    bandit_history = tuple([h for h in history if h == 'fight' or h == 'let_go'])
    #m =tuple(maze.reshape(-1).tolist())
